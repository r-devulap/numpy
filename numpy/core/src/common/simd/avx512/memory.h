...
#define npyv_load_f16(PTR) _mm512_loadu_ph(PTR)
#define npyvh_load_f16(PTR) _mm256_loadu_ph(PTR)
#endif
...
// unaligned store
#ifdef NPY_HAVE_AVX512_SPR
#define npyv_store_f16(PTR, data) _mm512_storeu_ph(PTR, data)
#define npyvh_store_f16(PTR, data) _mm256_storeu_ph(PTR, data)
#endif
...
{
    assert(nlane > 0);
    const __m256i vfill = _mm256_castph_si256(_mm256_set1_ph(fill));
    const __mmask16 mask = (0x0001 << nlane) - 0x0001;
    return _mm256_castsi256_ph(_mm256_mask_loadu_epi16(vfill, mask, ptr));
}
NPY_FINLINE npyvh_f16 npyvh_load_tillz_f16(const npy_half *ptr, npy_uintp nlane)
{
    assert(nlane > 0);
    const __mmask16 mask = (0x0001 << nlane) - 0x0001;
    return _mm256_castsi256_ph(_mm256_maskz_loadu_epi16(mask, ptr));
}
#endif
 ...
 *********************************/
#ifdef NPY_HAVE_AVX512_SPR
NPY_FINLINE npyvh_f16 npyvh_store_till_f16(npy_half *ptr, npy_uintp nlane, npyvh_f16 data)
{
    assert(nlane > 0);
    const __mmask16 mask = (0x0001 << nlane) - 0x0001;
    _mm256_mask_storeu_epi16(ptr, mask, _mm256_castph_si256(data));
}
#endif
//// 32
